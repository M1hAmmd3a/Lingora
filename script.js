...
/* No-email accounts: Firebase Auth still needs an email internally,
   so we build one deterministically from the username the person
   chooses. They never see or type an email anywhere. */
const EMAIL_DOMAIN = "lingoora.local";
function emailFromUsername(username){
  return username.toLowerCase() + "@" + EMAIL_DOMAIN;
}

/* ---------------- language table ---------------- */
// speech = locale for the browser's speech recognizer
// gt = code Google Translate expects (differs for Chinese)
const LANGS = {
  ar: { name: "العربية", speech: "ar-SA", gt: "ar" },
  en: { name: "English", speech: "en-US", gt: "en" },
  fr: { name: "Français", speech: "fr-FR", gt: "fr" },
  es: { name: "Español", speech: "es-ES", gt: "es" },
  tr: { name: "Türkçe", speech: "tr-TR", gt: "tr" },
  de: { name: "Deutsch", speech: "de-DE", gt: "de" },
  ru: { name: "Русский", speech: "ru-RU", gt: "ru" },
  zh: { name: "中文", speech: "zh-CN", gt: "zh-CN" },
};
...
/* ============================================================
   TRANSLATION — real machine translation, no placeholder text.
   Primary: Google's public translate endpoint (same one used by
   the "Google Translate" browser extension, no API key needed).
   Fallback: MyMemory, in case the primary is unreachable.
   ============================================================ */
async function translateGoogle(text, fromGt, toGt){
  const url = `https://translate.googleapis.com/translate_a/single?client=gtx&sl=${fromGt}&tl=${toGt}&dt=t&q=${encodeURIComponent(text)}`;
...
async function translateMyMemory(text, fromGt, toGt){
  const url = `https://api.mymemory.translated.net/get?q=${encodeURIComponent(text)}&langpair=${fromGt}|${toGt}`;
  const res = await fetch(url);
  if(!res.ok) throw new Error("mymemory http " + res.status);
  const data = await res.json();
  const t = data && data.responseData && data.responseData.translatedText;
  if(!t) throw new Error("mymemory: no translation returned");
  return t;
}

async function translateText(text, fromCode, toCode){
  if(!text.trim()) return { text: "", ok: true };
  if(fromCode === toCode) return { text, ok: true };
  const fromGt = (LANGS[fromCode] || {}).gt || fromCode;
  const toGt = (LANGS[toCode] || {}).gt || toCode;

  try{
    const translated = await translateGoogle(text, fromGt, toGt);
    return { text: translated, ok: true };
  }catch(e1){
    console.warn("Google translate failed, falling back to MyMemory:", e1.message);
    try{
      const translated = await translateMyMemory(text, fromGt, toGt);
      return { text: translated, ok: true };
    }catch(e2){
      console.error("MyMemory also failed:", e2.message);
...
function friendlyAuthError(err){
  const code = err.code || "";
  if(code.includes("email-already-in-use")) return "اسم المستخدم هذا محجوز، جرّب اسمًا آخر.";
  if(code.includes("invalid-email")) return "اسم المستخدم يحتوي على رموز غير مسموحة.";
  if(code.includes("weak-password")) return "كلمة المرور ضعيفة، استخدم 6 أحرف على الأقل.";
  if(code.includes("user-not-found") || code.includes("wrong-password") || code.includes("invalid-credential")) return "اسم المستخدم أو كلمة المرور غير صحيحة.";
  if(code.includes("permission-denied") || (err.message||"").includes("PERMISSION_DENIED")) return "لا توجد صلاحية للكتابة في قاعدة البيانات — تأكد من قواعد Realtime Database.";
...
document.getElementById("signupBtn").onclick = async () => {
  ...
  if(!name || !username || !pass){ authError.textContent = "الرجاء تعبئة كل الحقول."; return; }
  if(!USERNAME_RE.test(username)){
    authError.textContent = "اسم المستخدم يجب أن يكون حروف إنجليزية/أرقام/underscore فقط، 3 أحرف على الأقل.";
    return;
  }
...
document.getElementById("loginBtn").onclick = async () => {
  ...
  if(!username || !pass){ authError.textContent = "الرجاء تعبئة كل الحقول."; return; }
  const btn = document.getElementById("loginBtn");
  btn.disabled = true;
  try{
    await signInWithEmailAndPassword(auth, emailFromUsername(username), pass);
  }catch(err){
    authError.textContent = friendlyAuthError(err);
  }finally{
    btn.disabled = false;
...
async function sendMessage(){
  ...
  const result = await translateText(text, myLang, theirLang);
  status.textContent = result.ok ? "" : "تعذّرت الترجمة الآلية، أُرسل النص كما هو.";
...
if(SpeechRecognition){
  ...
  recognizer.onerror = (e) => {
    ...
    const status = document.getElementById("translateStatus");
    if(status) status.textContent = "تعذّر التقاط الصوت (" + (e.error || "خطأ") + ") — تأكد من صلاحية الميكروفون.";
  };
